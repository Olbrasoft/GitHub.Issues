...
builder.Services.AddScoped<ITitleTranslationNotifier, SignalRTitleTranslationNotifier>();

// Title translation service for progressive Czech translations on search results
    ...
    CancellationToken ct) =>
{
    ...
    logger.LogInformation("API: Starting title translation for {Count} issues: [{Ids}]",
        issueIds.Count, string.Join(", ", issueIds));

    // Fire and forget - translate each issue in background with its own DI scope
